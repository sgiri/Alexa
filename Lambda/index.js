...
const NameIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      .speak('Thanks' + name + ' how can I help you?')
      .reprompt('Thanks' + name + ' how can I help you?')
      .getResponse();
  },
};
...
canHandle(handlerInput) {
  ...
  // let prompts = handlerInput.requestEnvelope.request.intent.slots.searchitem.value;
   //let prompt = currentIntent.slots.category.value;
  ...
  //             .getResponse();
   ...
   return docClient.scan(params).promise().then((data) =>{
     ...
     var t = ' Database queried and ' + tableresults[0].name + 'has' + tableresults[0].brand + 'as favorite';
        ...
        if (currentSlot.confirmationStatus !== 'CONFIRMED'
          ...
          if (currentSlot.resolutions.resolutionsPerAuthority[0].status.code === 'ER_SUCCESS_MATCH') {
            if (currentSlot.resolutions.resolutionsPerAuthority[0].values.length > 0) {
                   if( currentSlot.name === 'searchitem' ){   
                 if(  currentIntent.slots.sortorder.name === 'sortorder' &&  currentIntent.slots.sortorder.resolutions && currentIntent.slots.sortorder.resolutions.resolutionsPerAuthority[0].status.code === 'ER_SUCCESS_MATCH'){
                   
                     const attributesManager = handlerInput.attributesManager;
                    const sessionAttributes = attributesManager.getSessionAttributes();
                    var temp = sessionAttributes.favoritebrand;
                   
                   prompt = 'Here is your '+  temp + 'results. Whole Hearted Grain Free Formula comes in a 25lb bag at a retail cost of 39 dollars and 99 cents.';
                ...
                .getResponse();
                   ...
                   prompt =  'How do you want it sorted?';
                ...
                else if( currentIntent.slots.category.name === 'category' && currentIntent.slots.category.resolutions && currentIntent.slots.category.resolutions.resolutionsPerAuthority[0]){
            
                  prompt =  'There are more than 36 results. what type of brand would you like?';
                 return handlerInput.responseBuilder
                .speak(prompt)
                .reprompt(prompt)
                ...
                .getResponse();
                 ...
                 prompt = 'What kind of dog food would you like ? You can say dry, wet, puppy food';
                 return handlerInput.responseBuilder
                .speak(prompt)
                .reprompt(prompt)
                .addElicitSlotDirective('category')
                .getResponse();
               } 
...
const CompletedSearchIntent = {
 ...
 handle(handlerInput) {
let outputSpeech = 'I found Merrick Brand';
 return handlerInput.responseBuilder
      .speak(outputSpeech)
      .getResponse();
},
